...
const getInactiveUsers = arr => arr.filter(i => i.isActive === true).map(i=> i.name);
...
const getUsersWithAge = (arr, min, max) => arr.filter(i => i.age >= min && i.age <= max).map(i=> i.name)
...
*/
// const getAllUniqueSkills = arr => arr.reduce((acc, arr) =>!arr.skills.includes(arr.skills) ? acc + arr.skills:null,0)
const getAllUniqueSkills = function(arr){
    let newArr = [];
arr.forEach((i)=> {
    if(!i.skills.includes(i.skills)){
        newArr.push(...i.skills);
    }
})
return newArr.sort()
}
...
*/
const getUserNamesSortedByFriendsCount = arr => arr.filter(i => i.friends).sort(sortBy).map(i=> i.name);
const sortBy = (a,b) => a.friends.length - b.friends.length;
